...
interface UniversitySelectorLightProps {
  onUniversityChange?: (university: UniversityOption | null) => void;
  className?: string;
}

export default function UniversitySelectorLight({ 
  onUniversityChange,
  className = "" 
}: UniversitySelectorLightProps) {
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <div className="relative">
        <button
          type="button"
          className="relative w-full cursor-pointer rounded-lg bg-white border border-gray-300 py-3 pl-3 pr-10 text-left text-gray-900 shadow-sm hover:border-gray-400 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-colors"
        ...
        >
          <span className="block truncate">
            {selectedUniversity ? (
              ...
              <span className="text-gray-500">Choose your campus...</span>
            )}
          </span>
        ...
        {isOpen && (
          <div className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-lg bg-white border border-gray-300 shadow-lg">
            <ul className="py-1">
              {universities.map((university) => (
                <li key={university.id}>
                  <button
                    type="button"
                    className="relative cursor-pointer select-none py-2 pl-3 pr-9 text-gray-900 hover:bg-gray-50 w-full text-left transition-colors"
